...
3. We need to get auto-differentiation through Python/Numba functions working, or fix the default.

4. We need to get delay differential equations working in Python. Again a type issue.

5. We need more comprehensive documentation and tutorials in the R and Python languages.

6. We need to cut down on JIT time. We can actually completely eliminate the JIT on the Julia side in wrapper functions (and precompile as well), but this hasn't been done yet.

7. We need to automate the installation more. Static compile the necessary Julia functions so it can run from R/Python without a Julia environment.
